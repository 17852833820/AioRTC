...
class FrameDecoder:
    timing_:VCMTiming = VCMTiming()
    jitter_buffer = JitterBuffer(capacity=128, is_video=True)
    ...
    def set_frame_rate(self,fps:int)->None:
        FrameDecoder.jitter_buffer._jitter_estimator.set_frame_rate(fps)
    ...
    def HasBadRenderTiming(self,now_ms:int,frame:JitterFrame)->bool:
        ...
        if abs(frame.render_time_ms -now_ms ) >kMaxVideoDelayMs:
            ...
            return True
        # with self.lock_:
        if FrameDecoder.timing_.target_video_delay()> kMaxVideoDelayMs:
    ...
    def find_next_frame(self,now_ms:int,frame:JitterFrame)->int:
        with self.lock_:
            ...
            if frame.render_time_ms == -1:
                frame.render_time_ms=FrameDecoder.timing_.render_time_ms(frame.timestamp,now_ms)
            wait_ms=FrameDecoder.timing_.max_waiting_time(frame.render_time_ms,now_ms)
    ...
    def get_next_frame(self,encoded_frame:JitterFrame)->JitterFrame:
        ...
        with self.lock_:
            if self.HasBadRenderTiming(now_ms,encoded_frame):
                FrameDecoder.jitter_buffer._jitter_estimator.reset()
                FrameDecoder.timing_.reset()
                render_time_ms=FrameDecoder.timing_.render_time_ms(encoded_frame.timestamp,now_ms)
            encoded_frame.render_time_ms=render_time_ms
            FrameDecoder.jitter_buffer._jitter_estimator.update_estimate(encoded_frame.frame_delay_ms,len(encoded_frame.data),False)
            jitterDelay=FrameDecoder.jitter_buffer._jitter_estimator.calculate_estimate() # 网络抖动
            encoded_frame.jitter_delay_ms=jitterDelay
            FrameDecoder.timing_.set_jitter_delay(jitterDelay)
            FrameDecoder.timing_.update_current_delay(render_time_ms,now_ms)
    ...
    async def decoder_worker(self, loop, input_q, output_q):
        ...
        while True:#任务处理无限循环
            ...
            if wait_time == -1:
                wait_time=0   
            if (encoded_frame.is_key_frame and encoded_frame.is_force_key):
                wait_time=0
            else:
                # 启动延迟执行的异步任务
            ...
            FrameDecoder.timing_.stop_decode_timer(dec_dur,clock.current_ms())
    ...
    async def delayed_decode(self,decoder,encoded_frame,wait_time,output_q,loop):
        ...
        for frame in decoder.decode(encoded_frame):
            if not encoded_frame.is_force_key:
                asyncio.run_coroutine_threadsafe(output_q.put(frame), loop) #将解码后的帧（frame）放入输出队列
            __log_debug('[DECODE] Add Render Frame...Stream id: {0}, PTS: {1},Time: {2}, Type: {3}'.format(encoded_frame.stream_id, frame.pts,frame.time, frame.pict_type))
        t2 = clock.current_ms()
        dec_dur =t2-t1
        if not encoded_frame.is_force_key:
...
class UniqueQueue(asyncio.Queue):
    ...
    def put_nowait(self, item):
        ...
        if item.pts > self.last_pts: # fix join bug
            ...
            logger.info("Put Frame to Render Queue. Number:{0}".format(item.pts))
        else:
            logger.info("Repeat Put Frame to Render Queue. Number:{0}".format(item.pts))
